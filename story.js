// ---------- STORYMAP FULLSCREEN with CURVAS + ZOOM CINEMÁTICO + DESENFOQUE + SLIDE IMAGES ----------
(function(){

  // ----------------------------------------------------
  // 1. CARGA DEL GEOJSON SUAVIZADO (Catmull–Rom A1)
  // ----------------------------------------------------
  let RUTAS_SUAVES = null;

  // Cargamos el archivo suavizado antes de iniciar el StoryMap
  fetch('rutas_suavizadas.geojson')
    .then(r => r.json())
    .then(data => {
      RUTAS_SUAVES = data;
      initStory();   // ← iniciamos el story sólo cuando ya tenemos las curvas
    });

  // ----------------------------------------------------
  // 2. INICIO PRINCIPAL DEL STORYMAP
  // ----------------------------------------------------
  function initStory(){
    ...
    const baseRoutes = L.geoJSON(RUTAS_SUAVES,{
      style:f=>({
    ...
    function activateChapter(ch){
      const feat = RUTAS_SUAVES.features.find(f=>f.properties.codigo===ch.codigo);
      if(!feat) return;
